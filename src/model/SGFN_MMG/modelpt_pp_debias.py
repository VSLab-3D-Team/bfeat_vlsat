...
from src.model.model_utils.network_RelFeatNet import RelFeatNaiveExtractor, RelFeatResNetConditionedFiLM
...
class Mmgnet(BaseModel):
    def __init__(self, config, num_obj_class, num_rel_class, dim_descriptor=11):
        ...
        self.obj_encoder = PointNetpp(num_dim=512, num_channel=9).to("cuda") # PointNetEncoder("cuda", channel=self.dim_point)   
    ...
    def forward(self, obj_points, edge_indices, descriptor=None, batch_ids=None, istrain=False):
        ...
        with torch.no_grad():
            obj_feature, _ = self.obj_encoder(obj_points)
        ...
        rel_pred_desc = self.proj_geo_desc(rel_feature_3d)
        ...
        if istrain:
            return obj_logits_3d, rel_cls_3d, gcn_edge_feature_3d_dis, logit_scale, rel_diff
        else:
            return obj_logits_3d, rel_cls_3d

    # Need to test: Despite its stochastic, it worth testing.
    # def calc_batch_ipw_weight(self, gt_obj_label, obj_single_occurrence, edge_indices, beta=0.7):
    #     """
    #     Inputs: 
    #         gt_obj_label: (B, ) Object Ground Truth label
    #         obj_single_occurrence: (N, ) Object Single Occurrence
    #         edge_indices: (B, 2) Edge Indices
    #     Output:
    #         loss weigth via IPW
    #     """
    #     N, B = obj_single_occurrence.shape[0], edge_indices.shape[0]
    #     obj_cooccur = torch.ones((N, N))
    #     sub_label, obj_label = self.index_get(gt_obj_label, edge_indices.t().contiguous()) 
    #     ipw_obj = torch.zeros((B,))
    #     for sub_idx, obj_idx in zip(sub_label, obj_label):
    #         obj_cooccur[sub_idx, obj_idx] += 1
    ...
    def process_train(self, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None, with_log=False, ignore_none_rel=False, weights_obj=None, weights_rel=None):
        self.iteration +=1    
        obj_logits_3d, rel_cls_3d, edge_feature_3d, obj_logit_scale, rel_diff = \
        ...
        if self.mconfig.multi_rel_outputs:
            ...
            pred_logit = torch.logit(rel_cls_3d) - tau * weights_rel[sub_label, obj_label, :].to("cuda")
