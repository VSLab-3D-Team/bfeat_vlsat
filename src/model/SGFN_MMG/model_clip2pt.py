...
from src.model.model_utils.network_DGCNN import DGCNN, DGCNN_partseg
from src.model.model_utils.network_CLIP2 import CLIP2Point, read_state_dict
...
class CLIP2Arg():
    def __init__(self, views, dim, model):
        self.views = views
        self.dim = dim
        self.model = model
...
class Mmgnet(BaseModel):
    def __init__(self, config, num_obj_class, num_rel_class, dim_descriptor=11):
        ...
        # Object Encoder        
        # self.obj_encoder = DGCNN(DGCNNArg(emb_dims=1024, k=15))
        # self.obj_encoder.load_state_dict(torch.load(self.mconfig.obj_pretrian_path, map_location='cuda:0'))
        # self.obj_encoder = self.obj_encoder.eval()
        
        self.clip2_args = CLIP2Arg(views=6, dim=512, model="PointNet")
        self.obj_encoder = CLIP2Point(self.clip2_args)
        self.obj_encoder.point_model.load_state_dict(read_state_dict(self.mconfig.obj_pretrian_path))
        self.obj_encoder = self.obj_encoder.eval()
        ...
        # self.proj_clip2 = torch.nn.Linear(1024, 512)
    ...
    def forward(self, obj_points, edge_indices, descriptor=None, batch_ids=None, istrain=False):
        ...
        with torch.no_grad():
            obj_feature = self.obj_encoder.infer(obj_points[:, :3, :].transpose(1, 2).contiguous())
        # obj_feature = self.proj_clip2(obj_feature)
        obj_features = rearrange(obj_feature, '(b n) d -> b n d', n=self.clip2_args.views)
        obj_feature = torch.mean(obj_features, dim=1) # B X N
