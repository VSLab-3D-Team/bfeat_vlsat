...
def read_json(split):
    ...
    selected_scans = set()
    if split == 'train' :
        selected_scans = selected_scans.union(read_txt_to_list('/data/hkh/sy/vlsat/data/3DSSG_subset/train_scans.txt'))
        with open("/data/hkh/sy/vlsat/data/3DSSG_subset/relationships_train.json", "r") as read_file:
            data = json.load(read_file)
    elif split == 'val':
        selected_scans = selected_scans.union(read_txt_to_list('/data/hkh/sy/vlsat/data/3DSSG_subset/validation_scans.txt'))
        with open("/data/hkh/sy/vlsat/data/3DSSG_subset/relationships_validation.json", "r") as read_file:
            data = json.load(read_file)
    else:
...
if __name__ == '__main__':
    ...
    with open('/data/hkh/sy/vlsat/data/3DSSG_subset/classes.txt') as f:
        obj_names = f.read().splitlines()
    with open('/data/hkh/sy/vlsat/data/3DSSG_subset/relationships.txt') as f:
        rel_name = f.read().splitlines()
    rel_name.pop(0)
    a = np.load('/data/hkh/sy/vlsat/config/results/Mmgnet/fill_mimic_ca/cls_matrix_list.npy')
    aa = np.load('/data/hkh/sy/vlsat/config/results/Mmgnet/fill_baseline_obj512/cls_matrix_list.npy')
    b = np.load('/data/hkh/sy/vlsat/config/results/Mmgnet/fill_mimic_ca/topk_triplet_list.npy')
    bb = np.load('/data/hkh/sy/vlsat/config/results/Mmgnet/fill_baseline_obj512/topk_triplet_list.npy')
